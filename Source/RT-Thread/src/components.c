 ...
 * 在系统启动后(本文件/rtthread_startup())，在板子初始化时(board/rt_hw_board_init)，调用了该函数。
 * 该函数调用了INIT_BOARD_EXPORT(fn)宏传入的函数。
 */
 ...
 * 在主线程入口(本文件/main_thread_entry),调用了该函数。
 * 该函数调用了INIT_*4个_EXPORT(fn)宏传入的函数。
 */
 ...
 * 在系统启动后(rtthread_startup())，调用了该函数（在rt_hw_board_init之后）
 */
